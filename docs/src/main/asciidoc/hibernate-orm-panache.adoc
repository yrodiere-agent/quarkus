...
For methods that only read data, you can use xref:transaction.adoc#read-only-transactions[read-only transactions] by adding `@io.quarkus.transaction.annotations.ReadOnly` alongside `@Transactional`.
This disables dirty checking and automatic flushing in Hibernate ORM, and sets the JDBC connection to read-only mode for potential server-side optimizations.
