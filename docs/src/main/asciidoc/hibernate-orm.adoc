...
A persistence unit (and its `EntityManagerFactory`) will be created when Quarkus detects that one is needed.
...
* A Hibernate ORM bean is injected (e.g. `EntityManager`, `Session`, `SessionFactory`).
...
* OR it is injected statically in application code (e.g. `@Inject Session` or `@Inject @PersistenceUnit("users") SessionFactory`)
