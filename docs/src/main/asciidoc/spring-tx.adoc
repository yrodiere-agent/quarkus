...
<3> `readOnly = true` enables xref:transaction.adoc#read-only-transactions[read-only transaction] optimizations.
...
|`readOnly`
|Supported
|Transformed into `@io.quarkus.transaction.annotations.ReadOnly`. See xref:transaction.adoc#read-only-transactions[read-only transactions].
