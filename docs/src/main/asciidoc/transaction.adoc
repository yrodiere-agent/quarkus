...
You can mark a transaction as read-only by adding the `@io.quarkus.transaction.annotations.ReadOnly` annotation alongside `@Transactional`:
...
A read-only transaction has the following effects:

* The JDBC connection is set to read-only mode (`Connection.setReadOnly(true)`).
Depending on the JDBC driver this may enable server-side optimizations: for example, PostgreSQL uses `BEGIN READ ONLY` to enforce read-only access at the transaction level, and MySQL sends `SET SESSION TRANSACTION READ ONLY`.
* If Hibernate ORM is used, the session is configured with `defaultReadOnly = true` and `FlushMode.MANUAL`, disabling dirty checking and automatic flushing.
* If Hibernate Reactive is used, the session is configured with `defaultReadOnly = true` and `FlushMode.MANUAL`, disabling dirty checking and automatic flushing.
* When the JDBC driver enforces read-only mode at the database level (PostgreSQL, MySQL, MariaDB, Oracle), the transaction is committed normally.
This allows Hibernate ORM to populate its second-level cache and transaction synchronization listeners to observe a committed status.
* When the JDBC driver does not enforce read-only mode (MSSQL, H2, and others), or when non-JDBC resources are enlisted, the transaction is rolled back as a safety net.
* Reactive transactions (Vert.x SQL Client) are always rolled back, since the reactive driver API does not support read-only enforcement.

NOTE: The adaptive commit/rollback behavior described above applies to declarative transactions (`@Transactional @ReadOnly`).
The programmatic API (`QuarkusTransaction.requiringNew().readOnly()`) currently always rolls back read-only transactions regardless of driver enforcement.
Calling `QuarkusTransaction.commit()` on a read-only transaction also results in a rollback.

Like `@TransactionConfiguration`, the `@ReadOnly` annotation can be placed on the method delineating the transaction boundary, or on the class to apply to all `@Transactional` methods of that class.

`@ReadOnly` must always be placed at the entry level of a transaction — the method that starts it.
Using `@ReadOnly` on a method that joins an existing transaction (e.g. `REQUIRED` when a transaction is already active, `SUPPORTS`, or `MANDATORY`) is not allowed and will result in a runtime error.
To nest a read-only transaction inside a read-write transaction, or vice versa, use `REQUIRES_NEW` so that each boundary starts its own transaction:
...
public void outerWrite() {
    // ... perform writes ...
    innerRead(); // suspends the write transaction, starts a new read-only one
}

@Transactional(Transactional.TxType.REQUIRES_NEW)
@ReadOnly
public List<Item> innerRead() {
    return Item.listAll();
}
----

The same applies in the other direction: a `@Transactional(REQUIRES_NEW)` method without `@ReadOnly` can start a read-write transaction while an outer read-only transaction is suspended.

Similarly, a non-read-only method joining a read-only transaction (e.g. via `REQUIRED` or `MANDATORY`) will result in a runtime error.
This follows the Jakarta Transactions specification, which requires a read-only transaction context for read-only methods and a non-read-only transaction context for non-read-only methods.

NOTE: `@ReadOnly` is a temporary annotation. It will be replaced by `@Transactional(readOnly = true)` once a future version of the Jakarta Transactions specification adds the `readOnly` attribute.
...
It depends on your DB and how many SQL SELECTs you are making. +
+
When your operation involves several SELECTs, it's better to wrap them in a single transaction because they will all be consistent with one another.
...
+
When your operation involves a single SELECT in a single datasource, then depending on your DB, an explicit transaction might indeed have a cost: the cost of the additional DB roundtrip to explicitly commit the transaction. +
Whether that cost is significant will depend on your DB and requirements. You should carefully consider whether a performance boost is worth the risk of unknowingly ending up with inconsistent operations (see above) when one day you introduce another SQL statement in the same operation. +
If the performance boost is not worth taking that risk, you should consider xref:transaction.adoc#read-only-transactions[read-only transactions], which give you transactional consistency while signaling the driver that no writes are expected, which can enable client-side and server-side optimizations.
+
If the performance boost _is_ worth taking that risk, then as a last resort you can just disable the transaction by marking your transaction boundary with `@Transactional(NOT_SUPPORTED)` (or `NEVER` or `SUPPORTS` depending on the semantic you want).
