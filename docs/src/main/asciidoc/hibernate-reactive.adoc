...
----
<1> Inject the reactive session directly
<2> Use `@Transactional` with `@io.quarkus.transaction.annotations.ReadOnly` for xref:transaction.adoc#read-only-transactions[read-only transactions], enabling optimizations
<3> Use `@Transactional` for persist operations - creates new entities in the database
<4> Use `@Transactional` for update operations - modifies existing entities
