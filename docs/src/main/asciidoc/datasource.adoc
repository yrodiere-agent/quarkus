...
When using xref:transaction.adoc#read-only-transactions[read-only transactions], the JDBC connection is automatically set to read-only mode (`Connection.setReadOnly(true)`) for the duration of the transaction.
Depending on the driver and database, this can enable client-side or server-side optimizations.

[NOTE]
====
Read-only transactions are guaranteed not to write to the database.

Behavior if your application does attempt to write (e.g. by issuing an `INSERT`) depends on the database and JDBC driver: the write may simply be ignored, or it may trigger an exception.
====
