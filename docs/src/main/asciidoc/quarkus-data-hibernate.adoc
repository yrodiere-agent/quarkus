...
TIP: For methods that only read data, you can use xref:transaction.adoc#read-only-transactions[read-only transactions] by adding `@io.quarkus.transaction.annotations.ReadOnly` alongside `@Transactional`.
