...
[TIP]
====
For queries that only read data, you can use xref:transaction.adoc#read-only-transactions[read-only transactions] by adding the `@io.quarkus.transaction.annotations.ReadOnly` annotation alongside `@Transactional`.

In a read-only transaction, Hibernate ORM automatically applies some optimizations, such as disabling dirty checking or flushes, on top of xref:datasource.adoc#narayana-transaction-manager-integration[optimizations applied at the datasource level].
====
